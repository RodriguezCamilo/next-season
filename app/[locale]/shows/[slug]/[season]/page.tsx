...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  return {
    title,
    description: desc,
    openGraph: { title, description: desc, images: [{ url: data.cover_url ?? '/og-default.png' }] },
    twitter: { card: 'summary_large_image', title, description: desc, images: [data.cover_url ?? '/og-default.png'] }
  };
}
...
export default async function Page({ params }: { params: Promise<Params> }) {
  ...
  await getTranslations({ locale, namespace: 'home' }); // (si necesitás strings de 'home', guardalo en t)
  ...
  return (
    <article className="mx-auto max-w-3xl p-6">
      <div className="relative aspect-[16/9] overflow-hidden rounded-xl border border-border bg-card">
        {data.cover_url && <Image src={data.cover_url} alt={`${data.item_title} cover`} fill className="object-cover" />}
      </div>
